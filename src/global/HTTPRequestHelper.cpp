...
NekoHTTPResponse NetworkRequestHelper::HttpGet(const QString &url) {
  ...
  QByteArray device_os = QSysInfo::productType().toUtf8();
  QByteArray ver_os = QSysInfo::productVersion().toUtf8();
  QByteArray device_model = QSysInfo::machineHostName().toUtf8();
  request.setRawHeader("x-hwid", hwid);
  request.setRawHeader("x-device-os", device_os);
  request.setRawHeader("x-ver-os", ver_os);
  request.setRawHeader("x-device-model", device_model);
  //
  ...
  //
  auto result =
      NekoHTTPResponse{_reply->error() == QNetworkReply::NetworkError::NoError
                           ? ""
                           : _reply->errorString(),
...
QString NetworkRequestHelper::GetHeader(
  ...
  for (const auto &p : header) {
    if (QString(p.first).toLower() == name.toLower())
      return p.second;
  }
  return "";
}
...
QString NetworkRequestHelper::DownloadAsset(const QString &url,
  ...
  connect(_reply, &QNetworkReply::downloadProgress, _reply,
          [&](qint64 bytesReceived, qint64 bytesTotal) {
            runOnUiThread([=] {
              GetMainWindow()->setDownloadReport(
                  DownloadProgressReport{fileName, bytesReceived, bytesTotal},
                  true);
